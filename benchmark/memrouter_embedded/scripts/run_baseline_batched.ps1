...
#
# Baseline Batched Evaluation Runner
# ===================================
# Runs Baseline eval in batches of N questions.
# Before each batch: cleans up and restarts OV Server + VikingBot.
# After each batch: checks for timeouts, retries if needed.
#
# Usage:
#   .\run_baseline_batched.ps1 -JudgeToken "sk-xxxxx"
#
# Optional:
...
# Config
$ovConf          = "$benchmarkRoot\config\ov_baseline.conf"
...
if ($FixedMasterDir) {
    ...
    $masterRunDir = "$benchmarkRoot\results\ablation\baseline_${timestamp}_batched"
}
...
function Invoke-BatchEval {
    ...
    $argList = @(
        "$scriptDir\eval_locomo_ov_with_memrouter_e2e.py",
        "--dataset", $dataset,
